    ...
    if hookfunction and getrenv and newcclosure then
        local _stbl; _stbl = hookfunction(getrenv().setmetatable, newcclosure(function(tbl, mt)
            if mt and typeof(mt) == "table" and rawget(mt, "__mode") == "kv" then
                ...
                if ok and tr and tr:find("MiscellaneousController") then
                    return _stbl({1,2,3}, {})
                end
            end
            return _stbl(tbl, mt)
...
do
    ...
    if getgenv then pcall(function() getgenv().InstanceFlushMovementState = flushAntiAimMovementState end) end
    ...
    function _G.SetUndergroundExt(state)
        ...
        if getgenv then pcall(function() getgenv().InstanceUndergroundEnabled = state end) end
        if state then
...
local S = {
    ...
    lightingAmbient = Color3.fromRGB(128, 128, 128),
    lightingOutdoorAmbient = Color3.fromRGB(128, 128, 128),
    lightingClockTime = 14,
    lightingFogEnd = 1000,
    lightingFogColor = Color3.fromRGB(192, 192, 192),
}
...
do
    local _wallbangFunc = function()
        ...
        local __a1b2c3 = setmetatable({}, {__index = function(_, s) return _cr(game:GetService(s)) end})
        local __p6q7r8 = getgenv and getgenv() or {}
...
do
    local function _createWallbangHead()
        local _cr2 = (typeof(cloneref) == "function") and cloneref or function(x) return x end
        local __a1b2c3 = setmetatable({}, {__index = function(_, s) return _cr2(game:GetService(s)) end})
        local __v2w3x4 = __a1b2c3.Players
        local __y5z6a7 = __a1b2c3.RunService
        local __b8c9d0 = __a1b2c3.ReplicatedStorage
...
local function resolveCameraRotationRemote()
    ...
    if r and r:IsA("RemoteEvent") then
        _cachedCameraRotationRemote = (typeof(cloneref) == "function") and cloneref(r) or r
        return _cachedCameraRotationRemote
    end
    return nil
end
...
AntiKatanaGroup:Toggle({
    ...
    Callback = function(v)
        if getgenv and getgenv().SetAntiKatana then getgenv().SetAntiKatana(v) end
    end
})
...
local WorldTab = Library:Tab("World", 6034763698)
local LightingGroup = WorldTab:Group("Lighting")

local _LightingService = game:GetService("Lighting")
local _LightingDefaults = {
    Brightness = _LightingService.Brightness,
    Ambient = _LightingService.Ambient,
    OutdoorAmbient = _LightingService.OutdoorAmbient,
    ClockTime = _LightingService.ClockTime,
    FogEnd = _LightingService.FogEnd,
    FogColor = _LightingService.FogColor,
}

local function applyLighting()
    if S.lightingEnabled then
        _LightingService.Brightness = S.lightingBrightness
        _LightingService.Ambient = S.lightingAmbient
        _LightingService.OutdoorAmbient = S.lightingOutdoorAmbient
        _LightingService.ClockTime = S.lightingClockTime
        _LightingService.FogEnd = S.lightingFogEnd
        _LightingService.FogColor = S.lightingFogColor
    else
        _LightingService.Brightness = _LightingDefaults.Brightness
        _LightingService.Ambient = _LightingDefaults.Ambient
        _LightingService.OutdoorAmbient = _LightingDefaults.OutdoorAmbient
        _LightingService.ClockTime = _LightingDefaults.ClockTime
        _LightingService.FogEnd = _LightingDefaults.FogEnd
        _LightingService.FogColor = _LightingDefaults.FogColor
    end
end

LightingGroup:Toggle({Name = "Enable Lighting Changer", Callback = function(v)
    S.lightingEnabled = v
    applyLighting()
end})
LightingGroup:Slider({Name = "Brightness", Min = 0, Max = 10, Default = 2, Rounding = 1, Callback = function(v)
    S.lightingBrightness = v
    if S.lightingEnabled then _LightingService.Brightness = v end
end})
LightingGroup:ColorPicker({Name = "Ambient Color", Default = Color3.fromRGB(128,128,128), Callback = function(c)
    S.lightingAmbient = c
    if S.lightingEnabled then _LightingService.Ambient = c end
end})
LightingGroup:ColorPicker({Name = "Outdoor Ambient", Default = Color3.fromRGB(128,128,128), Callback = function(c)
    S.lightingOutdoorAmbient = c
    if S.lightingEnabled then _LightingService.OutdoorAmbient = c end
end})
LightingGroup:Slider({Name = "Time of Day", Min = 0, Max = 24, Default = 14, Rounding = 1, Unit = " hrs", Callback = function(v)
    S.lightingClockTime = v
    if S.lightingEnabled then _LightingService.ClockTime = v end
end})
LightingGroup:Slider({Name = "Fog Distance", Min = 1, Max = 5000, Default = 1000, Rounding = 0, Unit = " studs", Callback = function(v)
    S.lightingFogEnd = v
    if S.lightingEnabled then _LightingService.FogEnd = v end
end})
LightingGroup:ColorPicker({Name = "Fog Color", Default = Color3.fromRGB(192,192,192), Callback = function(c)
    S.lightingFogColor = c
    if S.lightingEnabled then _LightingService.FogColor = c end
end})
