...
local function SetupBoosterCharacter(character)
    if not character then return end
    local humanoid = character:FindFirstChildOfClass("Humanoid") or character:WaitForChild("Humanoid", 10)
    if not humanoid then return end

    humanoid.Died:Connect(function()
        if not BoosterSettings.AutoRespawn then return end
        task.spawn(function()
            while BoosterSettings.AutoRespawn do
                local char = LocalPlayer and LocalPlayer.Character
                local currentHumanoid = char and char:FindFirstChildOfClass("Humanoid")
                if currentHumanoid and currentHumanoid.Health > 0 then break end
...
if LocalPlayer and LocalPlayer.Character then
    SetupBoosterCharacter(LocalPlayer.Character)
end
if LocalPlayer then
    LocalPlayer.CharacterAdded:Connect(SetupBoosterCharacter)
end

local function SetBoosterAutoHealth(enabled)
    BoosterSettings.AutoHealth = enabled == true
end

local function SetBoosterAutoAmmo(enabled)
    BoosterSettings.AutoAmmo = enabled == true
end

local function SetBoosterAutoRespawn(enabled)
    BoosterSettings.AutoRespawn = enabled == true
end

BoosterConnection = RunService.RenderStepped:Connect(function(dt)
    local character = LocalPlayer and LocalPlayer.Character
    if not character then return end
    local hrp = character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    BoosterT = BoosterT + (dt or 0) * 8
    local bounce = math.sin(BoosterT) * 4
    local targetPosition = hrp.Position + Vector3.new(0, bounce, 0)

    for _, obj in ipairs(workspace:GetChildren()) do
        if obj.Name == "_drop" and obj:IsA("BasePart") then
            local hasAmmo = BoosterSettings.AutoAmmo and obj:FindFirstChild("Ammo")
            local hasHealth = BoosterSettings.AutoHealth and obj:FindFirstChild("Health")
            if hasAmmo or hasHealth then
                pcall(function()
                    obj.Anchored = true
...
BoosterGroup:Toggle({Name = "Auto Health", Default = true, Callback = SetBoosterAutoHealth})
BoosterGroup:Toggle({Name = "Auto Ammo", Default = true, Callback = SetBoosterAutoAmmo})
BoosterGroup:Toggle({Name = "Auto Respawn", Default = true, Callback = SetBoosterAutoRespawn})
