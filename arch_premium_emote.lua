...
-- ═══════════════════════════════════════════════════════════════════
--  ARCH EMOTE ENGINE  (ported from Arch Script Emote standalone)
-- ═══════════════════════════════════════════════════════════════════
...
-- Emote state (merged into S table equivalents but kept separate for clarity)
local EmoteState = {
    enabled    = false,
    animId     = "",
    loop       = true,
...
local function emoteGetAnimator(char)
...
local function emoteResolveAnim(id)
...
local function emoteLoadPlay(char, id, spd, looped)
    if not char or id == "" then return nil end
    local animator = emoteGetAnimator(char)
    if not animator then return nil end
    local obj = emoteResolveAnim(id)
...
local function emoteStopAll()
    for _, t in ipairs(EmoteTracks) do
        pcall(function() t:Stop(0); t:Destroy() end)
    end
    EmoteTracks = {}
end

local function emotePlay()
    emoteStopAll()
    if not EmoteState.enabled or EmoteState.animId == "" then return end
    local char = LocalPlayer.Character
    if char then
        local t = emoteLoadPlay(char, EmoteState.animId, EmoteState.speed, EmoteState.loop)
        if t then table.insert(EmoteTracks, t) end
    end
    -- also drive the "Live" server replica if it exists (same pattern as standalone)
    local Live = workspace:FindFirstChild("Live")
    if Live then
        local sc = Live:FindFirstChild(LocalPlayer.Name)
        if sc then
            local t = emoteLoadPlay(sc, EmoteState.animId, EmoteState.speed, EmoteState.loop)
            if t then table.insert(EmoteTracks, t) end
        end
    end
end

-- Heartbeat: keep animation alive and adjust speed in real time
RunService.Heartbeat:Connect(function()
    if not EmoteState.enabled then return end
    if #EmoteTracks == 0 and EmoteState.animId ~= "" then
        emotePlay()
    end
    for _, t in ipairs(EmoteTracks) do
...
-- ─────────────────────────────────────────────────────────────────
--  EMOTE UI  —  Settings tab, right column
--  Group layout:
--    ConfigGroup  → Left  (already declared above)
--    EmotePresetsGroup → Right   (first group on right column)
--    EmoteOptionsGroup → Left    (alternates back, sits below Config)
--    EmoteCustomGroup  → Right   (below presets)
-- ─────────────────────────────────────────────────────────────────

-- ── Group 1 (Right): Preset buttons ─────────────────────────────
local EmotePresetsGroup = ConfigTab:Group("Emote Presets")

-- Each preset is a button. Clicking it plays the emote; clicking again stops it.
-- Active state tracked in EmoteState.presetName; notifications confirm the action.
for _, name in ipairs(EMOTE_PRESET_ORDER) do
    local presetName = name
    EmotePresetsGroup:Button({
        Name    = presetName,
        Tooltip = "Click to play / click again to stop: " .. presetName,
        Callback = function()
            if EmoteState.enabled and EmoteState.presetName == presetName then
                EmoteState.enabled    = false
                EmoteState.animId     = ""
                EmoteState.presetName = nil
                emoteStopAll()
                Library:Notify("Emote stopped", "warning")
            else
                EmoteState.enabled    = true
                EmoteState.presetName = presetName
                EmoteState.animId     = EMOTE_ANIM_PRESETS[presetName]
                emotePlay()
                Library:Notify("Emote: " .. presetName, "success")
            end
        end,
    })
end

-- ── Group 2 (Left): Emote options ────────────────────────────────
local EmoteOptionsGroup = ConfigTab:Group("Emote Options")

EmoteOptionsGroup:Toggle({
    Name    = "Loop",
    Default = EmoteState.loop,
    Tooltip = "Repeat the animation continuously",
    Callback = function(v)
        EmoteState.loop = v
        if EmoteState.enabled then emotePlay() end
    end,
})
...
EmoteOptionsGroup:Toggle({
    ...
    Tooltip = "Restart emote after respawn",
    Callback = function(v)
...
EmoteOptionsGroup:Button({
    Name    = "Stop Emote",
    Variant = "Danger",
    Tooltip = "Stop any playing emote",
    Callback = function()
        EmoteState.enabled    = false
        EmoteState.animId     = ""
        EmoteState.presetName = nil
...
-- ── Group 3 (Right): Custom ID ───────────────────────────────────
local EmoteCustomGroup = ConfigTab:Group("Custom Emote ID")

EmoteCustomGroup:Textbox({
    Name        = "Asset ID",
    Placeholder = "Enter animation asset ID...",
    Tooltip     = "Paste the numeric asset ID of any Roblox animation",
    Callback    = function(v)
        local id = v:match("%d+")
        if id then
            EmoteState.enabled    = true
            EmoteState.presetName = nil
            EmoteState.animId     = id
            emotePlay()
            Library:Notify("Custom emote loaded: " .. id, "success")
        end
    end,
})
...
EmoteCustomGroup:Button({
    Name    = "Play Custom ID",
    Variant = "Primary",
    Tooltip = "Play the asset ID entered above",
    Callback = function()
        if EmoteState.animId ~= "" then
            EmoteState.enabled = true
            emotePlay()
            Library:Notify("Custom emote playing", "success")
        else
            Library:Notify("Enter an asset ID first", "warning")
        end
    end,
})
