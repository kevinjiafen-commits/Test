-- RIVALS Rage / Melee / Backstab unified script
-- Complete extracted Ragebot kernel and Ragebot UI, packaged under one namespace.
...
local RagePackage = {}
RagePackage.Kernel = __kicia_hook_shared.rivals_ragebot_kernel
...
function RagePackage.Init(ctx)
    ctx = safeTable(ctx)

    local bridge = safeTable(ctx.RivalsRuntimeBridge)
    local options = safeTable(ctx.Options)
    local toggles = safeTable(ctx.Toggles)
    local fighterCache = safeTable(ctx.FighterDataCache)
    ...
    if type(genv.KiciaHookCaps) ~= "table" then
        genv.KiciaHookCaps = {
            has = function(name)
    ...
    elseif type(genv.KiciaHookCaps.gate) ~= "function" then
        genv.KiciaHookCaps.gate = function()
            return true
        end
    end

    local ok, result = pcall(function()
        return __kicia_hook_shared.rivals_ragebot_kernel().init({
            RivalsRuntimeBridge = bridge,
            Options = options,
            Toggles = toggles,
            FighterDataCache = fighterCache,
            GetChar = ctx.GetChar or getCharacter,
            GetRoot = ctx.GetRoot or getRoot,
            Genv = genv,
        })
    end)
    ...
    RagePackage.Bridge = bridge
    RagePackage.Options = options
    RagePackage.Toggles = toggles
    RagePackage.FighterDataCache = fighterCache

    return true, result
end

function RagePackage.Update(dt)
    local bridge = RagePackage.Bridge
    if type(bridge) ~= "table" then
        return false
    end

    local update = bridge.UpdateKiciaRagebot
    if type(update) ~= "function" then
        return false
    end

    return pcall(update, tonumber(dt) or 0)
end

function RagePackage.Reset()
    local bridge = RagePackage.Bridge
    if type(bridge) == "table" and type(bridge.ResetKiciaRagebot) == "function" then
        pcall(bridge.ResetKiciaRagebot)
    end
end

function RagePackage.Destroy()
    local bridge = RagePackage.Bridge
    if type(bridge) == "table" and type(bridge.DestroyKiciaRagebot) == "function" then
        pcall(bridge.DestroyKiciaRagebot)
    end
    RagePackage.Bridge = nil
end

local function BuildRivalsRagebotUi(AutomationTab)
...
RagePackage.BuildUI = BuildRivalsRagebotUi
