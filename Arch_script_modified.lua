...
local S = {
    ...
    espActive = false,
    autoHealthActive = false,
    autoAmmoActive = false,
    ...
    local _wallbangFunc = function()
        ...
        function obj:__find()
            ...
            if aliveEnemies == 0 then
                if Settings.NoVoidSpam ~= true and self.__hadEnemy then
...
function ApplyAllSettings()
    toggleAutoHealth(false)
    toggleAutoAmmo(false)
    ...
    if S.autoHealthActive then toggleAutoHealth(true) end
    if S.autoAmmoActive then toggleAutoAmmo(true) end
    if S.autoRespawnActive then toggleAutoRespawn(true) end
...
-- ===== AUTO HEALTH =====
local autoHealthConn
local function toggleAutoHealth(on)
    S.autoHealthActive = on
    if autoHealthConn then autoHealthConn:Disconnect(); autoHealthConn = nil end
    if on then
        autoHealthConn = RunService.Heartbeat:Connect(function()
            pcall(function()
                local char = LocalPlayer.Character
                if not char then return end
                local hum = char:FindFirstChildOfClass("Humanoid")
...
-- ===== AUTO AMMO =====
local autoAmmoConn
local function toggleAutoAmmo(on)
    S.autoAmmoActive = on
    if autoAmmoConn then autoAmmoConn:Disconnect(); autoAmmoConn = nil end
    if on then
        autoAmmoConn = RunService.Heartbeat:Connect(function()
            pcall(function()
                local fc = require(LocalPlayer.PlayerScripts.Controllers.FighterController)
                local fighter = fc and (fc.LocalFighter or (type(fc.GetFighter) == "function" and fc:GetFighter(LocalPlayer)))
                ...
                for _, item in pairs(items) do
                    if type(item) == "table" then
                        ...
                        if info and info.MaxAmmo then
                            pcall(function()
                                if type(item.Set) == "function" then
                                    item:Set("Ammo", info.MaxAmmo)
                                    item:Set("AmmoReserve", info.MaxAmmo * 5)
                                else
                                    item.Ammo = info.MaxAmmo
                                    item.AmmoReserve = info.MaxAmmo * 5
                                end
                            end)
                        end
                    end
    ...
    if on then
        autoRespawnConn = RunService.Heartbeat:Connect(function()
            pcall(function()
                ...
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum and hum.Health <= 0 then
                    -- Try game-native respawn remote first
                    local remote = ReplicatedStorage:FindFirstChild("Remotes")
                    remote = remote and remote:FindFirstChild("Data")
                    remote = remote and remote:FindFirstChild("Respawn")
                    if remote then
...
local ESPUtilGroup = Visuals:Group("Utilities")
ESPUtilGroup:Button({
    Name = "Auto Health",
    Tooltip = "Toggle: Lock local humanoid health to max every heartbeat",
    Variant = S.autoHealthActive and "Primary" or nil,
    Callback = function()
        local newState = not S.autoHealthActive
        toggleAutoHealth(newState)
        Library:Notify("Auto Health: " .. (newState and "ON" or "OFF"), newState and "success" or "warning")
    end
})
ESPUtilGroup:Button({
    Name = "Auto Ammo",
    Tooltip = "Toggle: Refill ammo and reserve on all equipped items every heartbeat",
    Variant = S.autoAmmoActive and "Primary" or nil,
    Callback = function()
        local newState = not S.autoAmmoActive
        toggleAutoAmmo(newState)
        Library:Notify("Auto Ammo: " .. (newState and "ON" or "OFF"), newState and "success" or "warning")
    end
})
ESPUtilGroup:Button({
    Name = "Auto Respawn",
    Tooltip = "Toggle: Automatically respawn on death using the game's respawn remote",
    Variant = S.autoRespawnActive and "Primary" or nil,
    Callback = function()
        local newState = not S.autoRespawnActive
        toggleAutoRespawn(newState)
        Library:Notify("Auto Respawn: " .. (newState and "ON" or "OFF"), newState and "success" or "warning")
    end
})
...
FlickbotGroup:Toggle({Name="No Void Spam",Default=false,Tooltip="Disable Ragebot's automatic void teleport when no enemies remain",Callback=function(v) if RageSettings then RageSettings.NoVoidSpam=(v == true) end end})
